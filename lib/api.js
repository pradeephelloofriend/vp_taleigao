...
export async function getProceedDataold(tabKey) {
  //console.log('tabKey-applo',tabKey)
  const data = await fetchAPI(`
  query getProceedData {
    pageBy(uri: "/proceedings/") {
      proceedings {
        dateAndTime
        description
        duration
  ...
  `)
  return data?.pageBy?.proceedings
}

export async function getProceedData(tabKey) {
  //console.log('tabKey-applo',tabKey)
  const data = await fetchAPI(`
  query getProceedData {
    proceedings {
